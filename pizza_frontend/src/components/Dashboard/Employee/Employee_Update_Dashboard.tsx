...
import { fetchEmployee, updateEmployee } from "../../store/employeeSlice";

const Employee_Update_Dashboard = () => {
    ...
    useEffect(() => {
        ...
        const fetchEmployeeUpdate = async () => {
            try {
                const employeeData = await dispatch(fetchEmployee(Number(id))).unwrap();

                if (employeeData?.Emp_Id) {
                    setFname(employeeData.Emp_Fname);
                    setLname(employeeData.Emp_Lname);
                    setHnumber(employeeData.Emp_Hnumber);
                    setAlley(employeeData.Emp_Alley);
                    setVillage(employeeData.Emp_Village);
                    setStreet(employeeData.Emp_Street);
                    setDistrict(employeeData.Emp_District);
                    setSubdistrict(employeeData.Emp_Subdistrict);
                    setProvince(employeeData.Emp_Province);
                    setPoscode(employeeData.Emp_Poscode);
                    setTel(employeeData.Emp_Tel);
                    setEmail(employeeData.Emp_Email);
                    setRole(employeeData.Emp_Role);
                    setWorkyear(employeeData.Emp_Workyear);
                    setSalary(employeeData.Emp_Salary);
                    setStatus(employeeData.Emp_Status);
                    setUsername(employeeData.Emp_Username);
                    setPassword(employeeData.Emp_Password);
                }
            } catch (err) {
    ...
    async function handlesubmit(e: React.FormEvent) {
        ...
        // รหัสผ่าน (ไม่บังคับสำหรับการแก้ไข - ถ้าไม่กรอกจะใช้รหัสผ่านเดิม)
        if (password.trim() !== "") {
        ...
        try {
            ...
            } else if (updateEmployee.rejected.match(res)) {
                let msg = res.payload || "ไม่สามารถอัปเดตข้อมูลพนักงานได้";

                if (msg === "มีชื่อผู้ใช้นี้ในระบบแล้ว") {
                    msg = "มีชื่อผู้ใช้นี้ในระบบแล้ว";
                }
                Swal.fire({
                    title: 'อัปเดตข้อมูลพนักงานไม่สำเร็จ!',
                    text: msg,
                    icon: 'error',
                    confirmButtonText: 'ตกลง',
                    confirmButtonColor: '#e7605b'
            ...
            Swal.fire({
                ...
                text: err?.message || "ไม่สามารถอัปเดตข้อมูลพนักงานได้",
                icon: "error",
                confirmButtonText: "ตกลง",
                confirmButtonColor: "#e7605b",
            });
                ...
                {employee?.Emp_Id ? (
                    <main className="flex flex-col w-full max-w-3xl bg-white shadow-xl rounded-2xl p-6 md:p-8">
                        <p className="font-bold text-2xl md:text-3xl text-gray-900 mb-6 md:mb-8">
                            แก้ไขข้อมูลพนักงานหมายเลข {employee.Emp_Id}
                        </p>
                        ...
                        <form className="flex flex-col gap-6 w-full" onSubmit={handlesubmit}>
                            ...
                            <div className="flex flex-col md:flex-row gap-4 w-full">
                                ...
                                <div className="w-full md:flex-1">
                                    ...
                                    />
                                    {erralley && <p className="text-red-500 text-xs mt-1">กรอกตรอก/ซอย</p>}
                                </div>
                            </div>
                            ...
                            <div className="w-full">
                                ...
                                <input
                                    id="password"
                                    type="password"
                                    placeholder="กรอกรหัสผ่านใหม่ (ไม่กรอกหากไม่ต้องการเปลี่ยน)"
                                    value={password}
                                    maxLength={15}
                                ...
                                <p className="text-slate-500 text-xs mt-1">*หากไม่ต้องการเปลี่ยนรหัสผ่าน ไม่ต้องกรอก</p>
                            </div>
...
export default Employee_Update_Dashboard;